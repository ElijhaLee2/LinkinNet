...
from model.Optimizer_1 import GeneratorOptimizer, DiscriminatorOptimizer
...
def build_whole_graph(batch):
    ...
    if IS_STACK_1:
        ...
        inter_rf_img = img + (
            tf.random_uniform(shape=[BATCH_SIZE, 1, 1, 1], minval=0., maxval=1.) * (generator_img.generated_pic - img))
        inter_rw_emb = embedding + (
            tf.random_uniform(shape=[BATCH_SIZE, 1], minval=0., maxval=1.) * (wrong_embedding-embedding))
        discriminator_gp_rf = Discriminator(inter_rf_img, embedding, 'dis_img', 'gp_rf', True)
        discriminator_gp_rw = Discriminator(img, inter_rw_emb, 'dis_img', 'gp_rw', True)
