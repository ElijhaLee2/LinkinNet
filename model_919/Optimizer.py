...
from model_919.hyperparameter import OPTIMIZER_DIS_HP, OPTIMIZER_GEN_HP
from model_919.vgg import compute_vgg_loss
...
class GeneratorOptimizer:
    ...
    def _get_loss(self):
        dis_fake = self.stack_to_train['dis_fake']
        vgg_r = self.stack_to_train['vgg_real']
        vgg_f = self.stack_to_train['vgg_fake']
        loss_dis = -tf.reduce_mean(dis_fake.score)
        loss_vgg = compute_vgg_loss(vgg_r, vgg_f, vgg_r.seg)
        loss = loss_dis + 0.5 * loss_vgg
