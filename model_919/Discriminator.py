...
class Discriminator:
    ...
    def _build_1(self, image_input, seg_input, text_embedding, variable_scope_name, additional_name, reuse):
        ...
        norm_params = DISCRIMINATOR_HP['norm_params']
        emb_reduced_dim = DISCRIMINATOR_HP['emb_reduced_dim']

        with tf.variable_scope(variable_scope_name, reuse=reuse):
            encoded_img = self.encode_img(image_input, df_dim, norm_fn, norm_params, act_fn, 0)
            encoded_seg = self.encode_seg(seg_input, df_dim, norm_fn, norm_params, act_fn, 0)
            ...
            # 4, 24*df
            # ATTENTION: ly_index !
            net = conv(concat, df_dim * 8, 1, 1, norm_fn, norm_params, act_fn, 7)
            # 4, 8*df
            score = conv(net, 1, 4, 1, None, None, None, 8, padding='VALID')  # 1,1 ,equal to fc

        return score  # (batch_size,)

    def encode_img(self, image_input, df_dim, norm_fn, norm_params, act_fn, start_index):
        with tf.variable_scope('encode_img'):
            net = image_input
            # 64,3*3
            net = conv(net, df_dim, 5, 2, None, None, act_fn, start_index)
            # 32,df
            net = conv(net, df_dim * 2, 4, 2, norm_fn, norm_params, act_fn, start_index + 1)
            # 16,2*df
            net = conv(net, df_dim * 4, 4, 2, norm_fn, norm_params, act_fn, start_index + 2)
            # 8,4*df
            net = conv(net, df_dim * 8, 4, 2, norm_fn, norm_params, None, start_index + 3)
            # 4,8*df
            net = residual_block(net, df_dim * 8, norm_fn, norm_params, act_fn, start_index + 4)
            net = act_fn(net)
            # 4,8*df=512
        return net

    def encode_seg(self, image_input, df_dim, norm_fn, norm_params, act_fn, start_index):
        with tf.variable_scope('encode_seg'):
            ...
            net = conv(net, df_dim * 4, 4, 2, norm_fn, norm_params, act_fn, start_index + 2)
            # 8,4*df
            net = conv(net, df_dim * 8, 4, 2, norm_fn, norm_params, act_fn, start_index + 3)
            # 4,8*df
    ...
    def wrap_emb(self, emb, emb_reduced_dim, tile_length, activation_fn):
        with tf.variable_scope('wrap_emb'):
            reduced_emb = dense(emb, emb_reduced_dim, None, None, activation_fn, '0_emb_fc')
